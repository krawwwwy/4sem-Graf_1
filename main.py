...
def generate_random_tree_with_nodes(num_nodes):
    ...
    values[0] = (0, random.randint(1, 10))  
...
def build_anytree(root, parent=None):
    if not root:
        return None
    node = Node(str(root.val), parent=parent)  
    if root.left:
...
def print_tree(root):
    ...
    if anytree_root:
        for pre, fill, node in RenderTree(anytree_root, style=AsciiStyle()):
